...
## Pre-committing tests

When adding new tests, you SHALL, if the test can be run without the patch, commit the test along with checks for existing behavior (autogenerated in any case where an update script exists in llvm/utils - `update_test_checks.py` for `opt`, `update_llc_test_checks.py` for assembly, and `update_mir_test_checks.py` for intermediate results) so that the commit with the changes clearly shows the impact of the change.
